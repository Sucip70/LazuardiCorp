...
export const COMPONENT_CATALOG: ComponentCatalogEntry[] = [
  ...
  },
  {
    ...
    label: 'Combobox',
    category: 'forms',
    description: 'Searchable dropdown — type to filter options, then pick one.',
    icon: '⌕',
    acceptsChildren: false,
    defaultProps: {
      name: 'combo',
    ...
    editableFields: [
      ...
      { key: 'options', label: 'Options (JSON)', type: 'json', group: 'content', required: true },
      { key: 'placeholder', label: 'Placeholder', type: 'text', group: 'content' },
      { key: 'emptyText', label: 'Empty result text', type: 'text', group: 'content' },
      { key: 'defaultValue', label: 'Default value', type: 'text', group: 'content' },
      {
        key: 'value',
        label: 'Value (bound)',
        type: 'text',
        group: 'behavior',
        helpText: 'One-way display only, e.g. {{choice}}. Leave empty for editable comboboxes.',
      },
      {
        key: 'bindToVar',
        label: 'Bind to variable',
        type: 'text',
        group: 'behavior',
        helpText: 'Two-way: selecting an option writes this variable.',
      },
      {
        key: 'bindScope',
        label: 'Bind scope',
    ...
    supportedEvents: [EVENT_PRESETS.change, EVENT_PRESETS.focus, EVENT_PRESETS.blur],
    a11yNotes: ['Uses combobox / listbox roles with keyboard Arrow/Enter/Escape support.'],
  },
  {
    type: 'Checkbox',
    label: 'Checkbox',
